...
class Conv1D:
    ...
    def forward(self, in_array: numpy.ndarray) -> numpy.ndarray:
        ...
        padded_input = numpy.pad(
            in_array, pad_width=self.padding, mode="constant", constant_values=0)
        ...
        for i in range(out_len):
            ...
            out_array[i] = numpy.sum(numpy.multiply(
                padded_input[idx:end], self.kernel))
...
class Conv2D:
    ...
    def forward(self, in_matrix: numpy.ndarray) -> numpy.ndarray:
        ...
        padded_input = numpy.pad(in_matrix, ((self.padding, self.padding), (
        ...
        for i in range(o_w):
            for j in range(o_h):
                ...
                out_matrix[i, j] = numpy.sum(
                    numpy.multiply(sub_matrix, self.kernel))
...
class Conv3D:
    ...
    def forward(self, in_tensor: numpy.ndarray) -> numpy.ndarray:
        ...
        padded_input = numpy.pad(in_tensor, ((self.padding, self.padding), (self.padding,
                                 self.padding), (self.padding, self.padding)), mode="constant", constant_values=0)
        ...
        strides = (self.stride * padded_input.strides[0], self.stride * padded_input.strides[1],
                   self.stride * padded_input.strides[2], padded_input[0], padded_input[1], padded_input[2])

        windows = as_strided(padded_input, shape=shape, strides=strides)

        out_tensor = numpy.tensordot(
            windows, self.kernel, axes=((3, 4, 5), (0, 1, 2)))
        return out_tensor
